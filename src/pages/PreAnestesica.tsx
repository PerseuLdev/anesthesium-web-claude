...
export function PreAnestesica() {
  // ── Tab state ──────────────────────────────────────────────────────────────
  const [activeTab, setActiveTab] = useState<'escores' | 'ficha' | 'medicacoes'>('escores');
  ...
  const [fichaASA, setFichaASA] = useState(ASA_CLASSES[0]);
  const [fichaEmergencia, setFichaEmergencia] = useState(false);
  const [fichaMallampati, setFichaMallampati] = useState(MALLAMPATI_CLASSES[0]);
  const [fichaULBT, setFichaULBT] = useState(1);

  const [fichaOBESE, setFichaOBESE] = useState({
    imcMaior26: false,
    barba: false,
    edentulo: false,
    roncoSaos: false,
  ...
  const [fichaLEMON, setFichaLEMON] = useState({
    aparenciaAnormal: false,
    regra332: false,
    mallampatiIIIIV: false,
    obstrucao: false,
  ...
  const [fichaRCRI, setFichaRCRI] = useState({
    cirurgiaAltoRisco: false,
    doencaIsquemica: false,
  ...
  const handleGerarFicha = async () => {
    ...
    try {
      const dados: DadosFichaAnestesica = {
        ...
        asaClass: fichaASA.id,
        emergencia: fichaEmergencia,
        mallampatiClass: fichaMallampati.id,
        ulbt: fichaULBT,
        lemonScore: fichaLemonScore,
        obeseScore: fichaObeseScore,
        rcriScore: fichaRCRIScore,
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">Escores, ficha integrada e medicações perioperatórias</p>
        </div>
      </div>
      ...
      <div className="flex p-1 bg-black/40 border border-white/5 rounded-xl">
        {[
          { id: 'escores', icon: ClipboardCheck, label: 'Escores' },
          { id: 'ficha', icon: User, label: 'Ficha Anestésica' },
      ...
      {activeTab === 'escores' && (
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {ASA_CLASSES.map((classe) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full flex-shrink-0 ${classe.cor}`} />
                      <div>
                        <p className={`font-bold ${asa.id === classe.id ? 'text-white' : 'text-slate-300'}`}>
                          ASA {classe.id}
                        </p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {MALLAMPATI_CLASSES.map((classe) => (
                      ...
                      >
                        <p className={`font-bold ${mallampati.id === classe.id ? 'text-blue-400' : 'text-slate-300'}`}>
                          Classe {classe.id}
                        </p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {CORMACK_LEHANE_CLASSES.map((classe) => (
                      ...
                      >
                        <p className={`font-bold ${cormack.id === classe.id ? 'text-blue-400' : 'text-slate-300'}`}>
                          Grau {classe.id}
                        </p>
                        <p className="text-xs text-slate-400 mt-1 line-clamp-2">{classe.descricao}</p>
                      </button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {(
                    ...
                    <CheckboxRow
                      ...
                      label={label}
                      checked={goldman[key]}
                      onChange={(v) => setGoldman({ ...goldman, [key]: v })}
                    />
                  ))}
                </div>
                <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
            ...
            <Card className="border-slate-800 bg-slate-900/50">
              <CardHeader>
                <CardTitle className="text-amber-400">Escore LEMON (Intubação Difícil)</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  />
                  <CheckboxRow
                    label="E — Regra 3-3-2 alterada (abertura boca, espaço hio-mentão, distância tireo-mento)"
                ...
                >
                  <div>
                    <p className="text-xs text-slate-500 uppercase tracking-wider">Score LEMON</p>
                    <p
                      className={`text-2xl font-bold ${
                        lemonScore >= 4 ? 'text-rose-400' : lemonScore >= 3 ? 'text-amber-400' : 'text-emerald-400'
                      }`}
                    >
                      {lemonScore}/5
                    </p>
                  ...
                  <div className="text-right">
                    ...
                    >
                      {lemonScore >= 4
                        ? 'Alto risco de intubação difícil'
                        : lemonScore >= 3
                        ? 'Risco moderado — atenção redobrada'
                        : 'Baixo risco'}
                    </p>
                    {lemonScore >= 3 && (
                      <p className="text-xs text-slate-400 mt-1">Considerar videolaringoscópio</p>
                    )}
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border-slate-800 bg-slate-900/50">
              <CardHeader>
                <CardTitle className="text-amber-400">Critérios OBESE (Ventilação Difícil)</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <CheckboxRow
                ...
                >
                  <div>
                    <p className="text-xs text-slate-500 uppercase tracking-wider">Critérios OBESE</p>
                    <p
                      className={`text-2xl font-bold ${
                        obeseScore >= 2 ? 'text-amber-400' : 'text-emerald-400'
                      }`}
                    >
                      {obeseScore}/5
                    </p>
                  ...
                  <div className="text-right">
                    ...
                    >
                      {obeseScore >= 2 ? 'Risco aumentado de ventilação difícil' : 'Baixo risco'}
                    </p>
                    {obeseScore >= 2 && (
                      <p className="text-xs text-slate-400 mt-1">Planejar máscara facial difícil</p>
                    )}
                  </div>
                </div>
              </CardContent>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {ASA_CLASSES.map((classe) => (
                  <button
                    key={classe.id}
                    onClick={() => setFichaASA(classe)}
                    className={`p-3 rounded-xl border text-left transition-all ${
                      fichaASA.id === classe.id
                        ? `border-${classe.cor.split('-')[1]}-500 bg-${classe.cor.split('-')[1]}-500/10`
                        : 'border-slate-800 hover:border-slate-700 bg-slate-800/30'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <p
                        className={`font-bold text-sm ${
                          fichaASA.id === classe.id ? 'text-white' : 'text-slate-300'
                        }`}
                      >
                        ASA {classe.id}
                      </p>
              ...
              <label className="flex items-center gap-3 p-3 rounded-xl border border-rose-500/20 bg-rose-950/20 cursor-pointer hover:bg-rose-950/30 transition-colors">
                ...
                <span className="text-sm font-semibold text-rose-300">
                  Cirurgia de Emergência (ASA {fichaASA.id}E)
                </span>
              </label>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {MALLAMPATI_CLASSES.map((classe) => (
                    <button
                      key={classe.id}
                      onClick={() => setFichaMallampati(classe)}
                      className={`p-3 rounded-xl border text-left transition-all ${
                        fichaMallampati.id === classe.id
                          ? 'border-blue-500 bg-blue-500/10'
                          : 'border-slate-800 hover:border-slate-700 bg-slate-800/30'
                      }`}
                    >
                      <p
                        className={`font-bold text-sm ${
                          fichaMallampati.id === classe.id ? 'text-blue-400' : 'text-slate-300'
                        }`}
                      >
                        Classe {classe.id}
                      </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <CheckboxRow
                    label="O — Obesidade (IMC > 26)"
                    checked={fichaOBESE.imcMaior26}
                    onChange={(v) => setFichaOBESE({ ...fichaOBESE, imcMaior26: v })}
                  />
                  <CheckboxRow
                    label="B — Barba presente"
                    checked={fichaOBESE.barba}
                    onChange={(v) => setFichaOBESE({ ...fichaOBESE, barba: v })}
                  />
                  <CheckboxRow
                    label="E — Edêntulo (sem dentes)"
                    checked={fichaOBESE.edentulo}
                    onChange={(v) => setFichaOBESE({ ...fichaOBESE, edentulo: v })}
                  />
                  <CheckboxRow
                    label="S — Ronco habitual / SAOS"
                    checked={fichaOBESE.roncoSaos}
                    onChange={(v) => setFichaOBESE({ ...fichaOBESE, roncoSaos: v })}
                  />
                  <CheckboxRow
                    label="E — Elderly (idade > 55 anos)"
                    checked={fichaOBESE.idadeMaior55}
                    onChange={(v) => setFichaOBESE({ ...fichaOBESE, idadeMaior55: v })}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <CheckboxRow
                    label="L — Aparência externa anormal"
                    checked={fichaLEMON.aparenciaAnormal}
                    onChange={(v) => setFichaLEMON({ ...fichaLEMON, aparenciaAnormal: v })}
                  />
                  <CheckboxRow
                    label="E — Regra 3-3-2 alterada"
                    checked={fichaLEMON.regra332}
                    onChange={(v) => setFichaLEMON({ ...fichaLEMON, regra332: v })}
                  />
                  <CheckboxRow
                    label="M — Mallampati III ou IV"
                    checked={fichaLEMON.mallampatiIIIIV}
                    onChange={(v) => setFichaLEMON({ ...fichaLEMON, mallampatiIIIIV: v })}
                  />
                  <CheckboxRow
                    label="O — Obstrução de via aérea / Obesidade"
                    checked={fichaLEMON.obstrucao}
                    onChange={(v) => setFichaLEMON({ ...fichaLEMON, obstrucao: v })}
                  />
                  <CheckboxRow
                    label="N — Mobilidade cervical reduzida"
                    checked={fichaLEMON.cervicalLimitada}
                    onChange={(v) => setFichaLEMON({ ...fichaLEMON, cervicalLimitada: v })}
                  />
                </div>
              </div>
              ...
              <AnimatePresence>
                {fichaLemonScore >= 3 && (
                  ...
                  >
                    <ShieldAlert className="w-4 h-4 text-rose-400 flex-shrink-0 mt-0.5" />
                    <p className="text-sm text-rose-300">
                      <span className="font-semibold">LEMON {fichaLemonScore}/5</span> — Videolaringoscópio
                ...
                {fichaObeseScore >= 2 && (
                  ...
                  >
                    <AlertTriangle className="w-4 h-4 text-amber-400 flex-shrink-0 mt-0.5" />
                    <p className="text-sm text-amber-300">
                      <span className="font-semibold">OBESE {fichaObeseScore}/5</span> — Risco aumentado de
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <CheckboxRow
                    ...
                    checked={fichaRCRI.cirurgiaAltoRisco}
                    onChange={(v) => setFichaRCRI({ ...fichaRCRI, cirurgiaAltoRisco: v })}
                  />
                  <CheckboxRow
                    label="Doença isquêmica cardíaca"
                    checked={fichaRCRI.doencaIsquemica}
                    onChange={(v) => setFichaRCRI({ ...fichaRCRI, doencaIsquemica: v })}
                  />
                  <CheckboxRow
                    label="Insuficiência cardíaca congestiva"
                    checked={fichaRCRI.insuficienciaCardiaca}
                    onChange={(v) => setFichaRCRI({ ...fichaRCRI, insuficienciaCardiaca: v })}
                  />
                  <CheckboxRow
                    label="Doença cerebrovascular (AVC/AIT)"
                    checked={fichaRCRI.doencaCerebrovascular}
                    onChange={(v) => setFichaRCRI({ ...fichaRCRI, doencaCerebrovascular: v })}
                  />
                  <CheckboxRow
                    label="Diabetes insulino-dependente"
                    checked={fichaRCRI.diabetesInsulina}
                    onChange={(v) => setFichaRCRI({ ...fichaRCRI, diabetesInsulina: v })}
                  />
                  <CheckboxRow
                    label="Creatinina > 2.0 mg/dL"
                    checked={fichaRCRI.creatinina2}
                    onChange={(v) => setFichaRCRI({ ...fichaRCRI, creatinina2: v })}
                  />
                </div>
                <div
          ...
          <Card className="border-blue-500/20 bg-blue-950/10">
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                <div className="p-3 rounded-xl bg-slate-900/50 border border-slate-800">
                  <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">ASA</p>
                ...
                <div className="p-3 rounded-xl bg-slate-900/50 border border-slate-800">
                  <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">OBESE</p>
                  <p className={`font-bold ${fichaObeseScore >= 2 ? 'text-amber-400' : 'text-white'}`}>
                    {fichaObeseScore}/5
                  </p>
                </div>
                <div className="p-3 rounded-xl bg-slate-900/50 border border-slate-800">
                  <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">RCRI</p>
                  <p
                    className={`font-bold ${
                      fichaRCRIScore >= 3
                        ? 'text-rose-400'
                ...
                <div className="p-3 rounded-xl bg-slate-900/50 border border-slate-800">
                  <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">METs</p>
                  <p className={`font-bold ${fichaMETs === 'baixo' ? 'text-amber-400' : 'text-emerald-400'}`}>
                    {fichaMETs === 'baixo' ? '≤ 4' : '> 4'}
                  </p>
                </div>
              </div>
              ...
              <AnimatePresence>
                {fichaAiSuggestion && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, height: 'auto' }}
                    exit={{ opacity: 0, height: 0 }}
                    className="p-4 rounded-xl bg-slate-950 border border-slate-700 prose prose-invert prose-sm max-w-none"
                  >
